...
flag=False
while flag==False:
    n=(input("enter 5 letter word:- ").lower())
    url="https://api.dictionaryapi.dev/api/v2/entries/en/"+n
    req=requests.get(url)
    d=req.json()
    if int(req.status_code)==404:
        print("this word does not exist in english language! this turn is cancelled try again")
        continue
    else:
        if len(n)!=5:
